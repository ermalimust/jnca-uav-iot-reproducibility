...
Download all parts for all checks. The `main54` check can run with `core` alone; P25 can run with `p25` alone. `verify_files.py` checks each installed v1.0.0 part; `replay_p25.py` checks the complete P25 inventory before replay. Supply `download_revision.py --cache PATH --offline` to reuse already downloaded archives without network access. Their sizes and hashes must match the pinned manifest.
