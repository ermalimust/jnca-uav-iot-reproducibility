// P21 spatial extension of P20 independent observation adapter. SPDX-License-Identifier: GPL-2.0-only
...
class PilotLoss : public PropagationLossModel {
 public:
  static TypeId GetTypeId(){static TypeId id=TypeId("ns3::P21SpatialLoss").SetParent<PropagationLossModel>().AddConstructor<PilotLoss>();return id;}
 ...
 private:
  double DoCalcRxPower(double tx,Ptr<MobilityModel> a,Ptr<MobilityModel> b) const override {
    ...
    if(target)out+=Spatial(Simulator::Now().GetSeconds());
...
int main(int argc,char** argv){
  uint32_t run=20001,nClients=1;std::string trajectory="",spatial="",mode="replay";double ref=40,eta=3,sigma=0,rho=.9,far=10000;
  bool noTraffic=false,pcap=false;double duration=12;
  CommandLine cmd(__FILE__);
  cmd.AddValue("spatial","Frozen spatial correction CSV",spatial);
