...
def build():
    ...
    with tempfile.TemporaryDirectory(prefix='p21_compile_') as tmp:
        t=Path(tmp).resolve();assert t.is_relative_to(Path(tempfile.gettempdir()).resolve()) and t.name.startswith('p21_compile_') and str(t).isascii()
...
def evaluate():
    ...
    def run(c):
        args=[f'--run={c["rng_run"]}',f'--clients={c["clients"]}',f'--trajectory={R/c["trajectory"]}',f'--spatial={R/c["spatial"]}',
