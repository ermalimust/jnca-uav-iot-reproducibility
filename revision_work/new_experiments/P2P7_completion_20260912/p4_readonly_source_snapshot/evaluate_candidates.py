...
METRICS=('oracle_coverage','near_oracle_coverage','best_candidate_regret','selected_regret','invalid_action_rate','fallback_rate','candidate_slots','raw_candidate_slots','unsupported_rate','posterior_admission_rate','empty_admission_rate','parse_success')
...
def evaluate_policy(policy,data,parse,cap):
    ...
    arrays=dict(oracle_coverage=coverage,near_oracle_coverage=near,best_candidate_regret=best,selected_regret=loss-data['oracle_loss'],invalid_action_rate=invalid.astype(float),fallback_rate=(selected==ACTIONS.index('FallbackProtect')).astype(float),candidate_slots=slots,raw_candidate_slots=raw_slots,unsupported_rate=unsupported,posterior_admission_rate=admitted,empty_admission_rate=empty,parse_success=np.full(n,float(parse)))
...
def main():
    ...
    for p in (HERE/'raw').glob('*.json'):
        o=json.loads(p.read_text(encoding='utf-8'));records[(o['variant'],o['replicate'],o['mission_id'])]=o
    ...
    for v in PROTOCOL['variants']:
        for rep in range(3):
            for mi,d in enumerate(data):
                ...
                generation.append(dict(variant=v,replicate=rep,mission=d['mission'],parse_success=o['parse_success'],schema_success=o['schema_check']['valid'],api_calls=len(o['calls']),prompt_tokens=sum(c.get('response',{}).get('usage',{}).get('prompt_tokens',0) for c in o['calls']),completion_tokens=sum(c.get('response',{}).get('usage',{}).get('completion_tokens',0) for c in o['calls']),tool_feedback_success=(o.get('tool_feedback') or {}).get('valid','')))
