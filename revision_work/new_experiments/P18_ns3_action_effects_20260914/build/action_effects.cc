// P18 independent Wi-Fi action-effects experiment. SPDX-License-Identifier: GPL-2.0-only
...
public:
  uint32_t id=0;
  static TypeId GetTypeId(){static TypeId t=TypeId("ns3::P18LedgerTag").SetParent<Tag>().AddConstructor<LedgerTag>();return t;}
...
struct Row {
  ...
  int64_t first_phy_ns=-1,last_phy_ns=-1,last_mac_drop_ns=-1; uint64_t drop_reason_mask=0;
};
...
static void Acked(Ptr<const WifiMpdu> mpdu){
  ...
  auto& r=rows.at(t.id);++r.mac_acks;if(mpdu->GetHeader().IsQosData())r.tid_mask|=1u<<mpdu->GetHeader().GetQosTid();
}
...
static void WriteRows(std::string path){
  ...
  out<<"packet_id,flow,payload_bytes,offer_ns,send_ns,receive_ns,shaper_drop_ns,socket_fail_ns,duplicates,phy_attempts,first_phy_ns,last_phy_ns,mac_acks,mac_drops,last_mac_drop_ns,drop_reason_mask,tid_mask\n";
  for(const auto& r:rows){if(!r.offered)continue;
    out<<r.id<<','<<r.flow<<','<<r.bytes<<','<<r.offer_ns<<','<<r.send_ns<<','<<r.receive_ns<<','<<r.shaper_drop_ns<<','<<r.socket_fail_ns<<','<<r.duplicates<<','<<r.phy_attempts<<','<<r.first_phy_ns<<','<<r.last_phy_ns<<','<<r.mac_acks<<','<<r.mac_drops<<','<<r.last_mac_drop_ns<<','<<r.drop_reason_mask<<','<<r.tid_mask<<'\n';
  }
}
...
int main(int argc,char** argv){
  ...
  cmd.AddValue("run","RNG run",run);cmd.AddValue("moving","M factor",mFactor);cmd.AddValue("zero","Engineering-only no traffic",zero);cmd.AddValue("pcap","Engineering PCAP",pcap);cmd.AddValue("distance","Engineering-only initial range",distance);cmd.Parse(argc,argv);
  NS_ABORT_MSG_IF(mFactor>1,"Invalid mobility factor");RngSeedManager::SetSeed(6091407);RngSeedManager::SetRun(run);
  ...
  if(mFactor){auto model=nodes.Get(1)->GetObject<ConstantVelocityMobilityModel>();Simulator::Schedule(Seconds(1),&ConstantVelocityMobilityModel::SetVelocity,model,Vector(3.5,0,0));Simulator::Schedule(Seconds(21),&ConstantVelocityMobilityModel::SetVelocity,model,Vector(0,0,0));}
  ...
  Simulator::Stop(Seconds(23));Simulator::Run();WriteRows(outputPath);
