// P24 capacity-parameter extension of sealed P23. GPL-2.0-only.
...
int main(int argc,char** argv){
 ...
 uint32_t width=20,nss=2;bool shortGi=false;
 CommandLine cmd(__FILE__);cmd.AddValue("width","Channel width MHz",width);cmd.AddValue("nss","Spatial streams",nss);cmd.AddValue("shortGi","Short guard interval",shortGi);cmd.AddValue("input","sim_second,rssi_dbm,packets CSV",path);
 ...
 cmd.AddValue("step","Engineering workload step",step);cmd.Parse(argc,argv);NS_ABORT_MSG_IF((width!=20&&width!=40)||nss<2||nss>4,"Invalid capacity config");
 ...
 YansWifiPhyHelper phy;phy.SetChannel(channel);phy.Set("ChannelSettings",StringValue("{6,"+std::to_string(width)+",BAND_2_4GHZ,0}"));
 phy.Set("TxPowerStart",DoubleValue(20));phy.Set("TxPowerEnd",DoubleValue(20));phy.Set("RxNoiseFigure",DoubleValue(7));
 phy.Set("Antennas",UintegerValue(nss));phy.Set("MaxSupportedTxSpatialStreams",UintegerValue(nss));phy.Set("MaxSupportedRxSpatialStreams",UintegerValue(nss));phy.SetErrorRateModel("ns3::NistErrorRateModel");
 WifiHelper wifi;wifi.SetStandard(WIFI_STANDARD_80211n);wifi.SetRemoteStationManager("ns3::MinstrelHtWifiManager");wifi.ConfigHtOptions("ShortGuardIntervalSupported",BooleanValue(shortGi));
 ...
 std::ofstream configOut("capacity_config.json");configOut<<"{\"width\":"<<width<<",\"nss\":"<<nss<<",\"short_gi\":"<<(shortGi?"true":"false")<<"}";
