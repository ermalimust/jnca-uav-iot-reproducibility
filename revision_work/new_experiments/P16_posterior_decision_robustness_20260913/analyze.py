...
def load(root):
    ...
    d={'q':q,'cost':co,'flags':fl,'ranks':ranks[idx],'h':h,'idx':idx,'mids':mids,'rows':rows,'lists':lists,'root':root,'policies':policies,'missions':missions}
...
def witness_search(d,cert):
    ...
    x=np.clip(q+hi[:,None]*direction,0,S);_,_,w=select(d,x)
    # Independent audit found ideal ties can round differently in the archived
    # NumPy code. Retain counterexamples only when both implementations agree.
    sys.path.insert(0,str(d['root']/REL))
    import paper7_agentic_feasibility as source_core
    import paper7_llm_candidate_experiment as source_router
    contexts=[]
    for mid in d['mids']:
        m=d['missions'][mid];g=m['guards']
        spec=source_core.MissionSpec(mid,m['intent'],m['gold_cost_profile'],{},*(bool(g.get(k,False)) for k in ('safety','rid','video','energy')))
        contexts.append((spec,source_core.cost_matrix(spec),d['policies'][mid]))
    def archived_winner(i):
        spec,cost,policy=contexts[d['idx'][i]];qq=x[i]/S
        return ACTIONS.index(source_core.guarded_select(source_router.candidate_actions_for(policy,qq),qq,cost,spec)[0])
    fp=np.array([archived_winner(i) for i in range(n)])
    bad=found&((fp!=w)|(fp==d['winner']));adjusted=int(bad.sum());steps=0
    while bad.any():
        hi[bad]+=1;assert np.all(hi[bad]<=S)
        x[bad]=np.clip(q[bad]+hi[bad,None]*direction[bad],0,S)
        _,_,w=select(d,x)
        for i in np.flatnonzero(bad):fp[i]=archived_winner(i)
        bad=found&((fp!=w)|(fp==d['winner']));steps+=1
        assert steps<=100,'Investigate rather than hide a nonlocal floating disagreement'
    norm=np.abs(x-q).max(1)
    ...
    return found,x,w,np.where(found,norm,S+1),{'initial_boundary_points_adjusted':adjusted,'maximum_extra_micro_steps':steps,'source_floating_witness_replays_passed':int(found.sum())}
...
def main():
    ...
    found,x,w,upper,fpcheck=witness_search(d,radii)
    pooled=[];mission=[];per={}
    for e in RADII:
    ...
    summary={'scope':'sufficient continuous-box certificate; witnessed upper bound; not an exact minimum flip radius',
      ...
      'floating_witness_check':fpcheck,
